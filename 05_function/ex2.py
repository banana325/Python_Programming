...
def swap(a,b):          #매개 변수는 객체의 참조값을 전달(같은 객체를 가리킴)
    print(id(a),id(b))
    a,b=b,a
    print(a,b)          # 2,1 > 함수 종료 시 지역 변수 a,b는 유효하지 않게 됨

a,b=1,2
print(id(a),id(b))
swap(a,b)               #a,b 객체의 참조값을 전달
print(a,b)              #1,2 > 전역 변수 a,b 출력

def append_item(num):   #num 매개 변수가 객체 참조값 복사
...
introduce("카리나", age=26, team="에스파", hometown="수원")
