...
result=["짝수" if i%2==0 else "홀수" for i in range(1,11)]
print(result)

# 각 이름의 길이로 이루어진 리스트 만들기
names=["pororo","crong","pobi","eddy"]
...
result = ["합격" if score>=60 else "불합격" for score in range(len(scores))]
print(result)                       # ✅ ['합격', '불합격', '합격', '불합격', '합격', '합격', '불합격', '합격']


# 3️⃣ 1 ~ 100 중 3 또는 5의 배수의 합 구하기 (sum() 함수 이용)
result = [i for i in range(1,101) if i%5==0 or i%3==0]
#result = [i for i in range(1,101) if not i%5 or not i%3] 위랑 같은거
...
# result = [w for w in words if w.count("n")]  위랑 같은거
...
# ii)
result = [round(sum(score)/len(score)) for score in zip(*scores)]
